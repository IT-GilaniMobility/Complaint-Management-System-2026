...
import { createComplaint, createCategory, fetchCategories } from "@/lib/supabase/queries";
import { addDays } from "date-fns";

const supabaseCategories = [
  { id: "00000000-0000-0000-0000-000000000011", name: "Technical Support" },
  { id: "00000000-0000-0000-0000-000000000012", name: "Billing & Payments" },
  { id: "00000000-0000-0000-0000-000000000013", name: "Product Quality" },
  { id: "00000000-0000-0000-0000-000000000014", name: "Customer Service" },
  { id: "00000000-0000-0000-0000-000000000015", name: "Shipping & Delivery" },
];
    ...
    setSubmitting(true);
      try {
        ...
        const labelByKey: Record<string, string> = {
          ...
          other: values.categoryOther?.trim() || "Other",
        };

        if (values.category in seededIds) {
          categoryId = seededIds[values.category as keyof typeof seededIds];
        } else {
          // Create category if not seeded; handle duplicates via unique name constraint
          try {
            const created = await createCategory({
              name: labelByKey[values.category],
              description: null,
              sla_hours: 48,
            } as any);
            categoryId = created.id as string;
          } catch (err) {
            // If duplicate, fetch categories and reuse existing by name
            const all = await fetchCategories();
            const match = all?.find((c: any) => c.name === labelByKey[values.category]);
            if (!match) throw err;
        ...
        const complaint = await createComplaint({
          ...
          reporter_id: "00000000-0000-0000-0000-000000000001", // Default to admin user
          assigned_to_id: null,
          customer_name: values.branch || "",
        ...
        toast({
          title: "Success",
          description: `Complaint ${complaint.complaint_number} created successfully.`,
        });
